...
- A **scheduled per-step prune** now runs at each turn boundary (streaming loop's Injection Point D and the headless `run_turn`), so an oversized tool result / screenshot added this step is shrunk *before* the next API call rather than surviving to re-summarization. It is a cheap per-node-cap pass (no aggregate surgery) and a no-op when everything is within caps — the "run prune on a cheap schedule (every step)" the takeaway calls for.

**Still open (deliberately not forced).** The scheduled per-step prune is now wired on the live turn paths. What remains as a deliberate follow-up is tuning the cadence and per-node caps by real-world observation (e.g. whether the 4000-char tool-result / 1024-char image caps want adjusting), and confirming the interaction between the per-step prune and the compaction manager's token accounting on very large sessions.

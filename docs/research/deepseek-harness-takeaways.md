...
This is the seam the later takeaways (durable inbox #11, derived title/usage, subagent/session projections) are meant to be built on. Nothing is wired into the live `Session` loop yet — `derive_messages` remains the on-demand source until consumers adopt the registry.

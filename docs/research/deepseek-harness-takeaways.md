...
**Still open (deliberately not forced).** The scheduled per-step prune is now wired on the live turn paths. What remains as a deliberate follow-up is tuning the cadence and per-node caps by real-world observation (e.g. whether the 4000-byte tool-result / 1024-byte image caps want adjusting), and confirming the interaction between the per-step prune and the compaction manager's token accounting on very large sessions.

...
- An **incremental seam** (`ProjectionRegistry::apply`) folds a single new event onto the running states in O(1), so producers append to the log and keep projections current without refolding the whole transcript — dsh's incremental-projection unit shape.

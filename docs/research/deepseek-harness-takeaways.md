...
**Current status (2026-09-19).** The projection registry is now in place as a first-class seam in `crates/jcode-base/src/session/invariants.rs`, matching dsh's `ctx.sessionProjections`:
- `ProjectionRegistry` folds *many* heterogeneous [`LogProjection`]s in a single pass (`fold`), exposing each typed derived state through a borrowing `ProjectionResults` view (dsh's `stateOf(key)` accessor). Registered states are erased into `Box<dyn Any + Send + Sync>` via boxed apply/validate closures, so unrelated state types (a `usize` counter next to a `Vec<StoredMessage>` transcript) fold together without a shared base type.
- An **incremental seam** (`ProjectionRegistry::apply`) folds a single new event onto the running states without re-walking earlier events, so producers append to the log and keep projections current without refolding the whole transcript — dsh's incremental-projection unit shape. Cost per event is projection-specific (O(1) for delta counters, O(span) for splice projections).
- Three built-in projections ship in `builtin()`: `MessageCountProjection` (live transcript length), the new `LiveTranscriptProjection` (the ordered `Vec<StoredMessage>` transcript incrementally, mirroring `derive_messages` splice semantics exactly), and `RoleCountsProjection`.
- A third built-in, `RoleCountsProjection`, demonstrates the "one fold, many readers" payoff concretely: it derives a *distinct* domain (per-role User/Assistant message counts) sharing the same single registry fold, keeping only a compact `Vec<Role>` (no full message payloads) and updating counts by *delta* per event — O(1) on append/insert, O(span) on a splice — with bounds shared via a canonical `SpliceBounds` helper so it never diverges from `derive_messages`. Its `validate` self-checks that the running totals match the folded `roles` list (catching a future edit that drifts the two hand-maintained fields). Tests prove one fold serves both domains, role counts stay correct across full/partial/reversed splices, and the self-consistency check flags a drifted state.
- Tests prove the incremental path (bootstrap fold + per-event `apply`) equals a fresh full re-fold and equals `derive_messages` on the same log.
- A `ProjectionMatchesDerived` invariant (wired into `InvariantRegistry::builtin()`) now proves the incremental `LiveTranscriptProjection` agrees byte-for-byte with `SessionEventMap::derive_messages()`. A differential test over every splice/insert edge (partial replace, clear+rebuild, reversed bounds, full replace) runs the invariant on each prefix. **This invariant immediately caught a real divergence**: for a *reversed* replace span (`start_index > end_index`), the projection's independent clamping had inserted the replacement at the wrong position, while `derive_messages` degrades the reversed span to a point-insertion at the trailing bound. The projection now mirrors `derive_messages`' clamping order exactly, so a future refactor that switches a hot path to read from the registry can prove it never diverges from the established source.

This is the seam the later takeaways (durable inbox #11, derived title/usage, subagent/session projections) are meant to be built on. The first producer has adopted it (see below); remaining on-demand `derive_messages` readers can migrate in turn, each safe because `ProjectionMatchesDerived` proves the projection never diverges.

The read path is now exposed on `Session` itself: `Session::projected_messages()` returns the transcript through the `LiveTranscriptProjection` fold (stated intent to consume *derived* state rather than scan the raw stream), with a test proving it mirrors `derive_messages` on a live session across append/insert. It is a *single-projection* accessor (via `project_map`); readers wanting message-count or per-role domains too should fold the `ProjectionRegistry` directly for one pass feeding many projections. It recomputes from the log on each call rather than keeping an incremental cache, deliberately — the map's `serde(skip)` cache philosophy warns against a second authoritative in-memory state, so the projection stays a pure derived convenience (proven byte-identical by `ProjectionMatchesDerived`) until a future optimization that keeps an incremental registry current at the append seam is justified by a measured hot path.

A live producer now consumes the seam: `Agent::seed_compaction_from_session` (jcode-app-core) reads the transcript via `Session::projected_messages()`, and `Agent::repair_missing_tool_outputs` (both its scan and its post-insert index recompute) likewise reads via the projection. `Session::projected_messages()` is **infallible** — `LiveTranscriptProjection` defines no failing validation, so it always returns the folded transcript (matching `derive_messages`; divergence is guarded by the `ProjectionMatchesDerived` invariant, not by a `None` return or a mask-the-bug fallback — a genuine internal failure would panic to surface the bug). Together these cover every production `derive_messages` reader in the agent loop, proving the seam in production (all 257 agent turn-path tests pass).
...
- A `/prune` slash command runs the model-free `PrunePolicy::node_caps()` pass on demand and reports `PruneReport`, complementing `/compact`.
- A **scheduled per-step prune** now runs at the top of both agent loops (before each next API call in the streaming loop and the headless `run_turn`), so an already-consumed oversized tool result / screenshot is shrunk *before* the next provider request rather than surviving to re-summarization. It is a cheap per-node-cap pass (no aggregate surgery) and a no-op when everything is within caps — the "run prune on a cheap schedule (every step)" the takeaway calls for. New (unconsumed) results and images stay intact until the model reads them once.

**Current status (2026-09-15).** The scheduled per-step prune and the compaction manager's token accounting now stay consistent:

- **Token-accounting interaction (closed).** The per-step prune runs at the top of both agent loops (`turn_loops.rs`, `turn_streaming_mpsc.rs`), *before* `messages_for_provider()`/`messages_for_api_with()` derives the messages and seeds the compaction manager, so auto-compaction always reads the already-pruned transcript. Confirmed against the manager's recency guard: a prune shrinks content in place **without changing the message count**, so `active_message_chars_with()` (which compares the cached mark against the observed active-message count) cannot detect the change on its own. Worse, the next `notify_message_added_blocks()` appends the assistant turn through the trusted `append_exact` fast path, silently re-validating any stale base. This was a real over-count bug: after a prune, the manager kept charging the pre-prune sizes (and the pre-prune `observed_input_tokens`), so auto-compaction could fire on a context that was actually already small. Fixed by `CompactionManager::note_prune_applied(all_messages)`, which recomputes (`set_exact`) the active-char estimate from the already-pruned transcript and clears `observed_input_tokens` — *recompute, not merely invalidate*, because invalidate alone is undone by the next append. Covered by the app-core streaming test `scheduled_prune_keeps_compaction_token_accounting_consistent_with_both_oversized_nodes` (both an oversized tool result and an oversized image) and the base-level `note_prune_applied_reseeds_from_pruned_transcript_before_append`.

- **Per-node caps (evidence-backed, tuned from real sessions).** The tool-result / image caps are deliberate *consumed-node retention* bounds. Tool results are bounded at the source — `bash` caps fresh output at `MAX_OUTPUT_LEN = 30_000` bytes and `read` at `MAX_READ_OUTPUT_CHARS = 120_000` chars. **The tool-result cap was raised from 4 KiB to 16 KiB based on real session data:** in a multi-turn coding session (Kotlin/Java task, DeepSeek-V4-Flash) ~20% of the 64 tool results were 4–18 KiB source-file reads (`IncomeDocumentListener.kt` at 17.7 KiB, repository/service files at 11–13 KiB) that are legitimately worth keeping whole; a 4 KiB cap cut them to head+tail and forced re-reads. 16 KiB retains the real reads (largest observed was 17.7 KiB, p90 ≈ 7.9 KiB) while still bounding larger dumps. The emergency-truncation cap (`EMERGENCY_TOOL_RESULT_MAX_CHARS`) stays 4 KiB as a separate harder recovery bound. Inline images are charged a flat `IMAGE_TOKEN_COST` (≈1600 tokens) regardless of payload — base64 length is deliberately *undercounted* to avoid spurious compaction — so the *token* win of pruning an image is modest (~1570 tokens) and the real win is raw-payload removal from the request body. Real screenshots measure ~35–104 KB of base64, so the 1024-byte cap rejects essentially every real screenshot; that is intended for the *per-node* pass on an **already-consumed** screenshot (the model has read it; replacing it with a marker — letting it re-capture if needed — is correct). 1024 is kept as the "consumed screenshot has no retained-request value" line, not a thumbnail budget.

- **Validation.** A new integration test drives a large transcript through the real streaming loop with **both** an oversized tool result and an oversized image in the consumed prefix, asserting the prune shrinks both nodes and that the manager's token estimate reseeds from the pruned (small) content, so a subsequent auto-compaction decides using the pruned sizes. The same reseed bug also existed on the **manual `/prune`** path: the app-core server route called only `note_compaction_applied()` (which resets provider/cache/tool state but not the manager's `active_chars`), so it kept a stale pre-prune over-count on the next auto-compaction gate. `request_manual_prune` now full-reseeds the compaction manager from the already-pruned transcript via `reseed_compaction_from_pruned_transcript()` (reset + set_budget + restore/seed), matching the TUI `/prune` handler's `reseed_compaction_from_provider_messages` and the 413 recovery path — so all user-invoked full-prune routes share the same semantics, while the cheap per-step prune keeps the lighter `note_prune_applied` recompute. Covered by `manual_prune_reseeds_compaction_token_accounting_from_pruned_transcript`, which was verified to fail (stale 12 659 vs pruned 923) against the pre-fix code. An edge case is also covered at the base level: `note_prune_applied` recomputes only the ACTIVE (uncompacted) suffix via `active_messages`, so a pre-existing compaction summary is NOT double-counted — the unit test `note_prune_applied_respects_existing_compaction_summary` locks that the estimate equals `summary chars + active (pruned) chars`.
...
- `crates/jcode-app-core/src/agent/guard.rs` implements a model-free detector (`repeat_reminder_from_transcript`) with a canonical, order-insensitive tool-signature comparison (nested JSON keys sorted), a configurable consecutive-identical rule, and a short `[Guard]` user-role nudge. Interleaved prose does not reset the run. The threshold is user-configurable via `[loop_guard] repeat_tool_threshold` (default 4; `0` disables the guard entirely), read by `repeat_reminder_threshold()` and wired into both loops.
...
- Detector unit tests cover sub-threshold silence, exact-threshold firing, configurable-lower-threshold firing, disable-at-zero, and nested-key canonicalization. A new end-to-end integration test (`streaming_turn_injects_repeat_tool_reminder_when_model_loops`) drives a real streaming turn with a provider that repeats the identical `bash` call the configured threshold times and asserts the guard injects exactly one `[Guard]` reminder naming the repeated tool and count, without ending the turn — proving the weld, not just the detector.

The per-call timeout half is implemented as an opt-in per-tool seam (`Tool::execution_timeout(input)` in `jcode-tool-core`, applied by `execute_with_deadline` at the registry wrap point so an exceeding call returns a clear, model-visible timeout error). Tools that can legitimately hang but are externally cancellable opt in — `bash` (with its own `timeout` param + background promotion), `session_search` (`execution_timeout`), `websearch`, `agentgrep` (foreground budget). A *generic* unconditional watchdog around `registry.execute` was deliberately not added as the default: many tools are legitimately long-running (interactive `ambient`, background `bash`), so per-tool declared timeouts are the safe surface rather than forcing every tool to declare an arbitrary cap.

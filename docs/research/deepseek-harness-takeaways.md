...
- Two built-in projections ship in `builtin()`: `MessageCountProjection` (live transcript length) and the new `LiveTranscriptProjection`, which derives the ordered `Vec<StoredMessage>` transcript incrementally, mirroring `SessionEventMap::derive_messages` splice semantics exactly.

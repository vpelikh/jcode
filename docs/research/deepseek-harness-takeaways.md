...
A live producer now consumes the seam: `Agent::seed_compaction_from_session` (jcode-app-core) reads the transcript via `Session::projected_messages()`, falling back to `derive_messages()` only if the projection hits an invariant violation. `Agent::repair_missing_tool_outputs` (both its scan and its post-insert index recompute) likewise reads via the projection. Together these cover every production `derive_messages` reader in the agent loop, proving the seam in production (all 257 agent turn-path tests pass).

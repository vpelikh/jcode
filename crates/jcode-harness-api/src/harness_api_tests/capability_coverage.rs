...
enum Disposition {
    ...
    #[allow(dead_code)] // Currently unused by the ledger; reserved for gaps worth exposing.
    Gap(&'static str),
}
...
const LEDGER: &[(&str, Disposition)] = &[
    ...
    ("Prune", Gap("Compact is Covered but its model-free prune sibling cannot be invoked by a client; a third-party client cannot shrink oversized tool results or images")),
    ...
    ("SetHandoffResume", ClientInternal),
    ("SetModel", Covered),
    ...
    ("SetWorkingDir", Gap("a client can set working_dir only at CreateSession bootstrap, not move a running session into a linked worktree or re-scope its tools")),

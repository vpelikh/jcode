...
/// A long bash command must wrap across continuation lines exactly like its
/// output, even on a wide terminal, so it never renders as one over-wide line
/// that the terminal soft-wraps or clips at the draw edge. Both the command and
/// output wrap at the same designed budget (BASH_WRAP_WIDTH).
#[test]
fn bash_command_wraps_like_output_across_continuation_lines() {
    crate::tui::ui::tools_ui::tests_tool_call_details_override::set(true);
    crate::tui::ui::tools_ui::tests_show_bash_output_override::set(true);
    let saved_centered = crate::tui::markdown::center_code_blocks();
    crate::tui::markdown::set_center_code_blocks(false);

    // A command long enough to cross the wrap budget even on a wide terminal.
    let command = format!(
        "cd /repo && git status && echo \"{}\" && cargo test -p jcode-tui --lib some::module::tests::{}",
        "0123456789 ".repeat(12),
        "fully_qualified_test_name_that_is_quite_long"
    );
    let output = format!("0123456789 {}", "0123456789 ".repeat(12));
    let content = format!(
        "{output}\nWorking directory: /repo\nExecution time: 10ms\nExit code: 0"
    );
    let msg = DisplayMessage {
        ...
        tool_data: Some(crate::message::ToolCall {
            ...
            name: "bash".to_string(),
            input: serde_json::json!({ "command": command, "cwd": "/repo" }),
            intent: Some("wrap test".to_string()),
            thought_signature: None,
        }),
    };

    // Wide terminal (e.g. 244 cols). BASH_WRAP_WIDTH caps both at 100 so the
    // command, like the output, must wrap into multiple 100-ish-col lines.
    let rendered = render_tool_message(&msg, 244, crate::config::DiffDisplayMode::Off)
        .iter()
        .map(extract_line_text)
        .collect::<Vec<_>>();

    // Command block lines carry the 4-space indent; the first begins "$ ", the
    // continuations are indented command text. Output lines use 6-space indent.
    let command_block: Vec<&String> = rendered
        .iter()
        .filter(|l| l.starts_with("    ") && !l.starts_with("      "))
        .filter(|l| {
            let t = l.trim_start().trim_start_matches("$ ").trim_start();
            !t.is_empty() && !t.starts_with("Output:")
        })
        .collect();
    let output_start = rendered
        .iter()
        .position(|l| l.trim_start().starts_with("Output:"))
        .unwrap_or(rendered.len());
    let output_block: Vec<&String> = rendered[output_start + 1..]
        .iter()
    ...
    assert!(
        command_block.len() > 1,
        "long command must wrap into multiple lines, got: {command_block:?}"
    );
    assert!(
        output_block.len() > 1,
        "long output must wrap into multiple lines, got: {output_block:?}"
    );

    // Every wrapped line (after its indent) stays within BASH_WRAP_WIDTH.
    for block in [&command_block, &output_block] {
        for l in block.iter() {
            let indent = if l.starts_with("      ") { 6 } else { 4 };
            assert!(
                unicode_width::UnicodeWidthStr::width(&l[indent..]) <= BASH_WRAP_WIDTH,
                "wrapped line exceeds BASH_WRAP_WIDTH: {l:?}"
            );
        }
    }

    // The entire command text is present across the wrapped lines.
    let joined = rendered.join("\n");
    let norm =
        |s: &str| s.chars().filter(|c| !c.is_whitespace()).collect::<String>();
    assert!(
        norm(&joined).contains(&norm(&command)),
        "full command text must be present across wrapped lines"
    );

    crate::tui::markdown::set_center_code_blocks(saved_centered);
    crate::tui::ui::tools_ui::tests_tool_call_details_override::set(false);
    crate::tui::ui::tools_ui::tests_show_bash_output_override::set(false);
}

...
#[test]
fn gate_budget_oscillating_gated_state_still_trips_the_breaker() {
    with_temp_jcode_home(|| {
        let mut app = create_test_app();
        app.auto_poke_incomplete_todos = true;
        ...
        crate::todo::save_todos(
            ...
            &[crate::todo::TodoItem {
                id: "todo-1".to_string(),
                content: "Oscillating workflow".to_string(),
                status: "completed".to_string(),
                priority: "high".to_string(),
                group: Some("release".to_string()),
                completion_confidence: Some(crate::todo::ConfidenceState::Verified),
        ...
        // An "involved" goal requires trade_off >= diligent. The model here
        // oscillates trade_off between None and Some(SomeConsidered) - both
        // still failing - turn after turn. A progress fingerprint that resets
        // on ANY gated-state difference would reward this oscillation with an
        // infinite budget. The recent-states window must instead refuse to
        // reset once the state returns to one it already tried, so the breaker
        // eventually trips and disarms.
        let goal = |trade_off: Option<crate::todo::TradeOffState>| crate::todo::TodoGoal {
            group: Some("release".to_string()),
            difficulty: Some(crate::todo::Difficulty::Involved),
            delivery_state: Some(crate::todo::DeliveryState::WorkflowValidated),
            autonomy: Some(crate::todo::Autonomy::NecessaryFollowthrough),
        ...
        crate::todo::save_goals(&app.session.id, &[goal(None)]).expect("save goal (None)");

        // Run many oscillation turns. Genuine per-turn alternation of the stuck
        // gate must not keep resetting the budget indefinitely.
        for i in 0..(App::TODO_COMPLETION_GATE_MAX_ATTEMPTS as usize + 6) {
            crate::todo::save_goals(
                &app.session.id,
                &[goal(if i % 2 == 0 {
                    None
                } else {
                    Some(crate::todo::TradeOffState::SomeConsidered)
                })],
            )
            .expect("save oscillating goal");
            app.schedule_auto_poke_followup_if_needed();
            app.queued_messages.clear();
            app.pending_queued_dispatch = false;
        }

        // Despite the oscillation, the gate must have exhausted its budget and
        // disarmed, proving the breaker is not defeated by A<->B alternation.
        assert!(
            !app.auto_poke_incomplete_todos,
            "oscillating a gated assessment must still disarm the circuit breaker"
        );
    });
}

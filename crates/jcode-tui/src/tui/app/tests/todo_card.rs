...
fn pinned_todos_hide_todo_tool_messages_from_the_transcript() {
    ...
    let prepared = crate::tui::ui::prepare::prepare_body(&app, 80, false);
    let body = prepared.wrapped_plain_lines.join("\n");
    assert!(!body.contains("duplicate todo transcript card"), "{body}");

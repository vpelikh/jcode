...
#[test]
fn test_streaming_prepare_cost_scales_with_text_length() {
    // Times the full-prep path (prepare_messages) with a growing streaming_text,
    // exercising the instrumented prepare_messages_inner. Prints the per-size
    // total so the O(streaming) hypothesis can be confirmed or refuted directly.
    let build_state = |n: usize| TestState {
        display_messages: vec![DisplayMessage::user("prompt"), DisplayMessage::assistant("base body line")],
        messages_version: 1,
        status: crate::tui::ProcessingStatus::Streaming,
        streaming_text: (0..n).map(|i| format!("token{}. ", i)).collect(),
        ..Default::default()
    };
    for n in [100usize, 500, 1000, 2000, 4000] {
        let state = build_state(n);
        let t = std::time::Instant::now();
        let _prepared = super::prepare::prepare_messages(&state, 100, 30);
        let ms = t.elapsed().as_secs_f64() * 1000.0;
        eprintln!("[stream-prepare] text_len={} prepare_ms={ms:.2}", n);
    }
}

#[test]
fn test_streaming_prepare_cost_body_size_split() {
    // Measures prepare_messages with a FIXED streaming tail but a growing body
    // (number of assistant messages). If prepare_ms grows with body size, the
    // dominant streaming-frame cost is the body/full-prep rebuild, not the small
    // streaming tail. This is the residual confirmation for the fix.
    let streaming = (0..1500usize).map(|i| format!("token{i}. ")).collect::<String>();
    let body_for = |n_msgs: usize| -> Vec<DisplayMessage> {
        let mut msgs = vec![DisplayMessage::user("prompt")];
        msgs.extend((0..n_msgs).map(|i| {
            DisplayMessage::assistant(format!(
                "message {i}: the quick brown fox jumps over the lazy dog and this line is fairly long so it wraps.\n```rust\nfn f_{i}() -> u32 {{ {i} }}\n```\n"
            ))
        }));
        msgs
    };
    for n_msgs in [2usize, 20, 60, 120] {
        let state = TestState {
            display_messages: body_for(n_msgs),
            messages_version: 1,
            status: crate::tui::ProcessingStatus::Streaming,
        ...
        let _prepared = super::prepare::prepare_messages(&state, 100, 30);
        let ms = t.elapsed().as_secs_f64() * 1000.0;
        eprintln!("[stream-prepare-body] msgs={n_msgs} prepare_ms={ms:.2}");
    }
}

...
#[test]
fn review_loop_record_surfaces_in_session_search() {
    with_temp_home(|home| {
        // Build a session with a review-loop state and a record.
        let mut session = Session::create_with_id("review-test".to_string(), None, None);
        session.short_name = Some("rt".to_string());
        session.working_dir = Some("/tmp/project".to_string());
        // A real post-completion session has at least one visible conversation
        // message; without it Session::save() does not write a snapshot.
        session.add_message(Role::User, vec![text("finished the work")]);

        // Create a review-loop state: one lens done with findings, a fix, and a record.
        use jcode_session_types::{
            Finding, ReviewLens, ReviewLoopState, ReviewRecord, ReviewRound,
        };
        let mut state = ReviewLoopState::new();
        state.current_lens = Some(ReviewLens::Correctness);
        state.record = Some(ReviewRecord {
            rounds: vec![ReviewRound {
                lens: ReviewLens::Correctness,
                findings: vec![Finding::new("HIGH", "src/foo.rs", "off by one")],
                clean: false,
                cant_fix: vec![],
            ...
            cant_fix: vec![],
            files_touched: vec!["src/foo.rs".to_string()],
            digest: Some("Review complete. ...".to_string()),
        });
        session.review_loop = Some(state);
        session.save().expect("save session");

        // Search for a term from the review record.
        let results = run_search(home, "off by one", &SearchOptions::for_test("test"));
        assert!(!results.is_empty(), "expected at least one result from review text");
        let result = &results[0];
        assert_eq!(result.source, "jcode");
        assert_eq!(result.session_id, "review-test");
        // The snippet should contain something from the review metadata.

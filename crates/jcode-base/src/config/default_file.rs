...
#                         # RECOMMENDED anti-censorship lever: set to a reverse-proxy Bot API
#                         # mirror that is reachable from your network when api.telegram.org is
#                         # blocked. When set, this mirror is the ONLY host jcode talks to:
#                         # startup probes it and every call goes to it, bypassing the
#                         # api.telegram.org auto-discovery sweep entirely. Env override:
#                         # JCODE_TELEGRAM_API_BASE
# telegram_proxy = ""     # Optional proxy for Telegram traffic, e.g. socks5://127.0.0.1:1080
#                         # or http://user:pass@host:3128. Applies whether or not a mirror is
#                         # set. Env override: JCODE_TELEGRAM_PROXY
# telegram_api_ip = ""    # Optional override: pin api.telegram.org to a specific DC IP
#                         # (e.g. 149.154.167.220) keeping its name for TLS. Only relevant when
#                         # NO telegram_api_base mirror is set (the mirror takes precedence and
#                         # ignores this). Usually leave empty: when the default endpoint is
#                         # blocked, jcode auto-discovers a reachable DC from a built-in
#                         # candidate list (led by the current api.telegram.org edge). Set this
#                         # only if that list is stale or you have a known-good IP.
#                         # Env: JCODE_TELEGRAM_API_IP

...
/// Fake SSE server: the first connection returns a 422 token-limit exhaustion
/// (matching the DeepSeek/OpenAI-compatible message shape), the second
/// connection (the retry) streams a clean, complete response.
fn spawn_token_limit_422_then_complete_server() -> String {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind fake provider server");
    let addr = listener.local_addr().expect("fake provider addr");

    std::thread::spawn(move || {
        // Connection 1: 422 token-limit error with a provider-suggested wait.
        {
            let (mut stream, _) = listener.accept().expect("accept first request");
            stream
                .set_read_timeout(Some(Duration::from_secs(5)))
            ...
            let mut request = vec![0u8; 65536];
            let _ = stream.read(&mut request);
            let body = r#"{"error":"Превышен лимит completion-токенов: использовано 60323, лимит 60000."}"#;
            let response = format!(
            ...
            stream
                .write_all(response.as_bytes())
                .expect("write token-limit 422 response");
        }
        ...
        // Connection 2 (the retry): clean complete response.
        {
            ...
            let _ = stream.read(&mut request);
            let body = concat!(
                "data: {\"choices\":[{\"delta\":{\"content\":\"answer after wait\"},\"finish_reason\":\"stop\"}]}\n\n",
                "data: [DONE]\n\n",
            );
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{}",
...
/// End-to-end acceptance for the token-limit 422 retry (Part 1): through the
/// real `run_stream_with_retries` runtime entry point, a 422 token-limit
/// exhaustion from an OpenAI-compatible endpoint must be retried (not surface
/// as a fatal error) and the retried request must deliver the successful
/// streamed response.
#[test]
fn token_limit_422_is_retried_and_recovers() {
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .expect("runtime");
    ...
    rt.block_on(async {
        let api_base = spawn_token_limit_422_then_complete_server();
        ...
        while let Some(item) = rx.recv().await {
            match item {
                Ok(StreamEvent::TextDelta(text)) if text.contains("answer after wait") => {
                    saw_answer = true;
                }
                Ok(_) => {}
                Err(e) => {
                    eprintln!("retry loop surfaced a fatal error: {e:#}");
        ...
        assert!(
            !saw_fatal_error,
            "a token-limit 422 must be retried, not surfaced as a fatal error"
        );
        assert!(
            saw_answer,
            "the retried request must deliver the successful streamed response"
        );
    });
}

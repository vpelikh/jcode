...
impl Editor {
    ...
    pub fn delete_word_back(&mut self) {
        // An active selection is replaced, matching delete_back/delete_forward
        // and normal text-field behavior. Without this the drain below shortens
        // the buffer while `anchor` keeps pointing past the new end, and the
        // next Backspace slices out of bounds (#728).
        if self.delete_selection().is_some() {
            return;
        }
